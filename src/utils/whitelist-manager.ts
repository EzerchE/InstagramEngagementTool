...
export const exportWhitelist = (whitelistedUsers: readonly UserNode[]): void => {
  if (whitelistedUsers.length === 0) {
    alert('No protected accounts to export');
    return;
  }
  ...
  link.href = url;
  link.download = `engagement-guard-protected-${new Date().toISOString().split('T')[0]}.json`;
...
export const clearWhitelist = (): void => {
  if (!confirm('Clear the entire protected account list? This action cannot be undone.')) {
    return;
  }
